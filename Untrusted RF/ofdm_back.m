...
bc_signal_fh = ones(n_ofdm/2,1);%返回全为1的矩阵----information bit B = 1
bc_signal_bh = ones(n_ofdm/2,1)-2;%返回全为-1的矩阵----information bit B = -1
...
padding_bc = zeros(sym_rem,1);   %pad zero补零
bc_signal = [bc_signal;padding_bc];
size(bc_signal);
size(data);
back_data = alpha*bc_signal.*data; %%为什么要乘bc_signal??
