...
sym_rem_d = mod(n_ofdm-mod(length(y_d),n_ofdm),n_ofdm);
padding = repmat(0+0i,sym_rem_d,1);
y_d_padded = [y_d;padding];

sym_rem_b = mod(n_ofdm-mod(length(y_b),n_ofdm),n_ofdm);
padding = repmat(0+0i,sym_rem_b,1);
