function  [v_y1_cp, v_y2_cp,H,H_t] = generate_key_bit_CCA_Original_Model(data_ofdm, n_ofdm, n_cp ,n_frame,channelStrength)
...
% H = h.*channelStrength;
% 
% H_t = h_t.*channelStrength;

H = ray_model(d(3),taps(3)).*channelStrength;

H_t = ray_model_cor(H,cor,d(3));

% % H_t = randi(channelStrength,d(3),1);

% all transmissions in ambient backscatter communication
y1_d = ofdm_trans(data_ofdm, h+H, pw_noise);
y1_d_t = ofdm_trans(data_ofdm, h_t+H_t, pw_noise);
