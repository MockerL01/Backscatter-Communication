function  [v_y1_cp, v_y2_cp, v_key1, v_key2,H1,H2,H_12] = generate_key_bit_CCA(data_ofdm, n_ofdm, n_cp ,n_frame,channelStrength,H1_flag,H2_flag,H12_flag)
...
y1_d_design = ofdm_trans(data_ofdm,h1+H1,pw_noise);
back1_design = ofdm_back(y1_d_design,alpha,n_ofdm,n_frame);

y2_d_design = ofdm_trans(data_ofdm,h2+H2,pw_noise);
back2_design = ofdm_back(y2_d_design,alpha,n_ofdm,n_frame);

y1_b_design = ofdm_trans(back2_design,h_12+H_12,pw_noise);
y2_b_design = ofdm_trans(back1_design,h_12+H_12,pw_noise);
...
y2_b_channel = ofdm_trans(y1_d_channel,h_12+H_12,pw_noise);
